import React, { useContext, useState } from 'react';
import { ModalContext } from '../../context/ModalContext';
import Modal from '@material-ui/core/Modal';
import { makeStyles } from '@material-ui/core/styles';

function getModalStyle() {
    const top = 50;
    const left = 50;

    return {
        top: `${top}%`,
        left: `${left}%`,
        transform: `translate(-${top}%, -${left}%)`,
    };
}

const useStyles = makeStyles((theme) => ({
    paper: {
        position: 'absolute',
        width: 450,
        backgroundColor: theme.palette.background.paper,
        boxShadow: theme.shadows[5],
        padding: theme.spacing(2, 4, 3),
    },
}));

const Recetas = ({ receta }) => {
    const [modalStyle] = useState(getModalStyle);
    const [open, setOpen] = useState(false);
    const { setIdDrink, informacion } = useContext(ModalContext);
    const classes = useStyles();
    const { idDrink, strDrink, strDrinkThumb } = receta;

    const handleOpen = () => {
        setOpen(true);
    };

    const handleClose = () => {
        // setIdDrink(null);
        setOpen(false);
    };

    const guardarIdDrink = () => {
        setIdDrink(idDrink);
        handleOpen();
    };

    const mostrarIgredientes = (data) => {
        let ingredientes = [];

        for (let i = 1; i < 16; i++) {
            if (data[`strIngredient${i}`] !== null) {
                ingredientes.push(
                    <li key={i}>
                        {data[`strIngredient${i}`]} {data[`strMeasure${i}`]}
                    </li>,
                );
            }
        }

        return ingredientes;
    };

    return (
        <div className='col-md-4 mb-3'>
            <div className='card'>
                <h2 className='card-header'>{strDrink}</h2>
                <img
                    src={strDrinkThumb}
                    alt={strDrink}
                    className='card-img-top'
                />
                <div className='card-body'>
                    <button
                        type='button'
                        className='btn btn-block btn-primary mt-2 mb-2'
                        onClick={guardarIdDrink}
                    >
                        Ver receta
                    </button>
                    <Modal
                        open={open}
                        onClose={() => {
                            setIdDrink(null);
                            handleClose();
                        }}
                    >
                        <div style={modalStyle} className={classes.paper}>
                            <h2>{informacion.strDrink}</h2>
                            <h3 className='mt-4'>Instrucciones</h3>
                            <p>{informacion.strInstructions}</p>

                            <img
                                className='img-fluid my-4'
                                src={informacion.strDrinkThumb}
                                alt={informacion.strDrink}
                            />

                            <h3>Ingredientes y cantidades</h3>
                            <ul>{mostrarIgredientes(informacion)}</ul>
                        </div>
                    </Modal>
                </div>
            </div>
        </div>
    );
};

export default Recetas;
